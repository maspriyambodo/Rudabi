...
<input type="hidden" name="jum_lsmislam"/>
<input type="hidden" name="milik_sekolah"/>
<input type="hidden" name="milik_wakaf"/>
<input type="hidden" name="milik_yayasan"/>
<input type="hidden" name="milik_pemerintah"/>
<input type="hidden" name="luas_tanah"/>
<input type="hidden" name="luas_bangunan"/>
<input type="hidden" name="pgrs_pria"/>
<input type="hidden" name="pgrs_perempuan"/>
<script>
    window.onload = function () {
        var url = "https://simas.kemenag.go.id/rudabi/datapi/simpenaiss/lsmislam?KEY=BOBA&province_id=<?= $id; ?>";
        $('table').dataTable({
            "ServerSide": true,
            "order": [[0, "asc"]],
            "paging": false,
            ...
            columns: [
                ...
                {data: "jum_lsmislam", className: "text-center sum_orm"},
                ...
                {
                    data: null, className: "text-center sum_lt",
                    render: function (data) {
                        var luas_tanah = data.luas_tanah;
                        var a = luas_tanah.replace('-', '');
                        return a;
                    }
                },
                {
                    data: null, className: "text-center sum_lb",
                    render: function (data) {
                        var luas_bangunan = data.luas_bangunan;
                        var a = luas_bangunan.replace('-', '');
                        return a;
                    }
                },
                {data: "pgrs_pria", className: "text-center sum_pria"},
                {data: "pgrs_perempuan", className: "text-center sum_wani"}
            ],
            footerCallback: function () {
                ...
                api.columns('.sum_orm', {page: 'current'}).every(function () {
                    ...
                    $('input[name=jum_lsmislam]').val(numFormat(sum));
                ...
                api.columns('.sum_sek', {page: 'current'}).every(function () {
                    ...
                    $('input[name=milik_sekolah]').val(numFormat(sum));
                    $(this.footer()).html(numFormat(sum));
                });
                api.columns('.sum_wak', {page: 'current'}).every(function () {
                    ...
                    $('input[name=milik_wakaf]').val(numFormat(sum));
                    $(this.footer()).html(numFormat(sum));
                });
                api.columns('.sum_yay', {page: 'current'}).every(function () {
                    ...
                    $('input[name=milik_yayasan]').val(numFormat(sum));
                    $(this.footer()).html(numFormat(sum));
                });
                api.columns('.sum_pem', {page: 'current'}).every(function () {
                    ...
                    $('input[name=milik_pemerintah]').val(numFormat(sum));
                    $(this.footer()).html(numFormat(sum));
                });
                ...
                api.columns('.sum_pria', {page: 'current'}).every(function () {
                    var sum = this
                            .data()
                            .reduce(function (a, b) {
                                var x = parseFloat(a) || 0;
                ...
                api.columns('.sum_wani', {page: 'current'}).every(function () {
                    var sum = this
                            .data()
                            .reduce(function (a, b) {
                                var x = parseFloat(a) || 0;
        ...
        am4core.ready(function () {
            ...
            var data = [
                ...
                {
                    city_title: "Milik Sekolah",
                    jumlah: b
                },
                {
                    city_title: "Milik Wakaf",
                    jumlah: c
                },
                {
                    city_title: "Milik Yayasan",
                    jumlah: d
                },
                {
                    city_title: "Milik Pemerintah",
                    jumlah: e
                }
            ];
