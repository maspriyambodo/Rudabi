<style>
    ...
    #chartdiv_a,#chartdiv_b,#chartdiv_pendidikan{
...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
    <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        <div class="d-flex align-items-center flex-wrap mr-2">
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5 text-uppercase">data dewan hakim provinsi {provinsi}</h5>
        </div>
    </div>
</div>
...
<div class="card card-custom">
    ...
    <div class="card-body">
        ...
        <div id="chartdiv_pendidikan"></div>
        <hr class="clear" style="margin:5% 0px;">
...
<script>
    var url = "https://simas.kemenag.go.id/rudabi/datapi/simpenaiss/dewan?province_id=<?= $id; ?>";
    ...
    am4core.ready(function () {
        ...
        var valueAxis = chart.yAxes.push(new am4charts.ValueAxis());
        valueAxis.renderer.minWidth = 100;
        valueAxis.title.text = "Jumlah Pendidikan Dewan Hakim";
        valueAxis.title.fontWeight = 800;
        var series = chart.series.push(new am4charts.ColumnSeries());
        series.dataFields.valueY = "jumlah";
        series.dataFields.categoryX = "pendidikan";
        series.clustered = false;
        series.tooltipText = "Pendidikan Dewan Hakim {categoryX}: [bold]{valueY}[/]";
        var hoverState = series.columns.template.column.states.create("hover");
        hoverState.properties.cornerRadiusTopLeft = 0;
        hoverState.properties.cornerRadiusTopRight = 0;
        hoverState.properties.fillOpacity = 1;
    ...
    $('table').dataTable({
        ...
        columns: [
            {
                ...
                render: function (data) {
                    ...
                    b = data.city_title;
                    c = b.replace(' ', '_');
                    return '<a href="<?= base_url('PAI/Dewan/Kabupaten/'); ?>' + a + "/" + c + '">' + b + '</a>';
                }
            },
            {data: "jum_dewan", className: "text-center sum_dw"},
            {data: "jum_pria", className: "text-center sum_pria"},
            {data: "jum_perempuan", className: "text-center sum_per"},
            {data: "status_kawin", className: "text-center sum_kaw"},
            {data: "status_belum_kawin", className: "text-center sum_blm"},
        ...
        footerCallback: function () {
            ...
            api.columns('.sum_dw', {page: 'all'}).every(function () {
                ...
                $('input[name=dewantxt]').val(numFormat(sum));
                document.getElementById('title_chartdiv_a').innerHTML = "<b>Jumlah Dewan Hakim Provinsi " + "<?= $provinsi; ?> " + numFormat(sum) + "</b>";
            ...
            api.columns('.sum_pria', {page: 'all'}).every(function () {
                ...
                $('input[name=lakitxt]').val(numFormat(sum));
                $(this.footer()).html(numFormat(sum));
            });
            api.columns('.sum_per', {page: 'all'}).every(function () {
                ...
                $('input[name=perempuantxt]').val(numFormat(sum));
                $(this.footer()).html(numFormat(sum));
            });
            api.columns('.sum_kaw', {page: 'all'}).every(function () {
                ...
                $('input[name=nikahtxt]').val(numFormat(sum));
                $(this.footer()).html(numFormat(sum));
            });
            ...
            api.columns('.sum_pes', {page: 'all'}).every(function () {
                ...
                $('input[name=pesantrentxt]').val(numFormat(sum));
                $(this.footer()).html(numFormat(sum));
            });
