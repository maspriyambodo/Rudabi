...
class Bimwin extends CI_Controller {
    ...
    public function Provinsi($id, $prov) {
        $data = [
            ...
            'data' => $this->bodo->Curel("https://simas.kemenag.go.id/rudabi/datapi/embimwin/targetcatin2020?KEY=BOBA&id_prop=" . $id . "")
        ];
    ...
    public function Targetcatin($id) {
        ...
        if (strlen($id) <= 99) {
            ...
            $data = $this->bodo->Curel("https://simas.kemenag.go.id/rudabi/datapi/embimwin/targetcatin2020?KEY=BOBA&id_prop=" . $dec . "");
        }
        $this->output
